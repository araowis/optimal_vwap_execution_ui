...
const FEED_AUTHORIZE_URL = 'https://api.upstox.com/v3/feed/market-data-feed/authorize';
...
function parseMode(mode: string | null): 'ltpc' | 'full' | 'option_greeks' | 'full_d30' {
  if (mode === 'ltpc' || mode === 'full' || mode === 'option_greeks' || mode === 'full_d30') return mode;
  return 'full';
}
...
export async function GET(req: NextRequest) {
  ...
  if (!instrumentKey) {
    return new Response(JSON.stringify({ ok: false, error: 'Missing instrument_key' }), {
      status: 400,
      headers: { 'content-type': 'application/json' },
    });
  }
  ...
  if (!authorizeResp.ok) {
    return new Response(JSON.stringify({ ok: false, status: authorizeResp.status, error: authorizeText }), {
      status: 200,
      headers: { 'content-type': 'application/json' },
    });
  }
  ...
  const wsUrl: string | undefined = authorizeData?.data?.authorized_redirect_uri || authorizeData?.data?.authorizedRedirectUri;

  if (!wsUrl) {
    return new Response(JSON.stringify({ ok: false, error: 'Authorize response missing websocket url' }), {
      status: 200,
      headers: { 'content-type': 'application/json' },
    });
  }
      ...
      try {
        ...
        ws.on('message', (data: WebSocket.RawData) => {
          try {
            ...
            const msg = e instanceof Error ? e.message : 'Failed to decode protobuf message';
            sendJsonLine({ type: 'decode_error', error: msg });
          }
        });

        ws.on('close', (code: number, reason: Buffer) => {
          sendJsonLine({ type: 'connection_status', status: 'close', code, reason: reason?.toString?.() });
          safeClose();
        });

        ws.on('error', (err: Error) => {
          const msg = err instanceof Error ? err.message : 'WebSocket error';
          sendJsonLine({ type: 'connection_status', status: 'error', error: msg });
          safeError(err);
        });
