  ...
  candles.forEach((candle) => {
    ...
    if (candleRange === 0) {
      ...
      for (let i = 0; i < numLevels; i++) {
        const levelKey = Math.floor((low + (i * candleRange) / numLevels - minPrice) / levelSize);
        if (levelKey >= 0 && levelKey < numLevels) {
          volumeByLevel.set(levelKey, (volumeByLevel.get(levelKey) || 0) + volumePerLevel);
        }
      }
    }
  });
...
export function getVolumeAtPrice(
  ...
  return bins
    .filter((bin) => (above ? bin.priceLevel >= price : bin.priceLevel <= price))
