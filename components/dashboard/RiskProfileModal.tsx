...
export default function RiskProfileModal({
  ...
  const isProtected = editingProfile ? PROTECTED_PROFILES.includes(editingProfile.profileId || '') : false;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!profileId || !displayName || defaultLambda === '' || defaultNBins === '') return;
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-background/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-card border border-border w-full max-w-lg rounded-sm shadow-md overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {editingProfile ? 'Modify execution risk settings' : 'Define new execution behavior'}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          <form id="risk-profile-form" onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      value={profileId}
                      onChange={(e) => setProfileId(e.target.value.toUpperCase().replace(/\s+/g, '_'))}
                  ...
                  {editingProfile && <p className="text-[10px] text-muted-foreground">Profile ID is immutable after creation.</p>}
                </div>
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-medium text-muted-foreground">
                      Default Lambda (Risk Aversion) <span className="text-destructive">*</span>
                    </label>
                    <span className="text-xs font-mono bg-secondary px-2 py-0.5 rounded-sm">{defaultLambda}</span>
                  </div>
                  <input
                    type="range"
                    min="1"
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-medium text-muted-foreground">
                      Default N Bins (Execution Chunks) <span className="text-destructive">*</span>
                    </label>
                    <span className="text-xs font-mono bg-secondary px-2 py-0.5 rounded-sm">{defaultNBins}</span>
                  </div>
                  <input
                    type="range"
                    min="4"
          ...
          <div>
            ...
            {isProtected && (
              <p className="text-xs text-muted-foreground italic flex items-center gap-1">
                <Lock className="w-3.5 h-3.5" /> Built-in profiles cannot be modified.
              </p>
            )}
          </div>
          <div className="flex items-center gap-3">
