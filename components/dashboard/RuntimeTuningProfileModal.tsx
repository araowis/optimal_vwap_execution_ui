...
type NumericField = Omit<
  RuntimeTuningProfile,
  "id" | "profileName" | "description" | "createdAt" | "updatedAt"
>;
...
const PARAM_GROUPS: ParamGroupDef[] = [
  ...
  {
    label: "Adjustment Clamps",
    accent: "text-emerald-400",
    fields: ["adjustmentClampLo", "adjustmentClampHi"],
  },
  {
    label: "Trend Weights",
  ...
  },
  {
    label: "Scaling",
    accent: "text-emerald-400",
    fields: ["barStructAlpha", "scaleFactorHi", "scaleFactorLo"],
  },
  {
    label: "Liquidity",
...
const FIELD_STEPS: Partial<Record<keyof NumericField, number>> = {
  upTrendConsecBars: 1,
  downTrendConsecBars: 1,
  refinementPasses: 1,
  binSmoothingWindow: 1,
  minSpacingDenominator: 1,
};
  ...
  useEffect(() => {
    ...
    if (editingProfile) {
      ...
      (Object.keys(DEFAULT_PARAMS) as (keyof NumericField)[]).forEach((k) => {
        (p as any)[k] =
          (editingProfile as any)[k] ?? (DEFAULT_PARAMS as any)[k];
      });
      setParams(p);
    } else {
      setProfileName("");
