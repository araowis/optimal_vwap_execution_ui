...
const ChartPanel = memo(function ChartPanel({
  ...
  useEffect(() => {
    console.log(
      "Pretrade fetch triggered. instrumentKey:",
      instrumentKey,
      "mode:",
      mode,
    );
    if (!instrumentKey || mode === "backtest") {
      console.log(
        "No instrumentKey or mode is backtest, clearing pretrade data",
      );
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">
                {filteredCandles.length} candles ({sampledDisplayData.length}{" "}
                displayed) • {customizationPrefs.chartPeriod}
              </span>
              {vwapWsConnected ? (
