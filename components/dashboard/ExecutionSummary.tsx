...
export default function ExecutionSummary({
  ...
  if (dailyResult && dailyResult.status === "OK") {
    ...
    const fillPct = r.totalQty
      ? ((r.executedQty ?? 0) / r.totalQty) * 100
      : 100;
      ...
      <div className="border-t border-border bg-card px-6 py-4">
        <div className="grid grid-cols-12 gap-4 items-center">
          ...
          <div className="col-span-2 flex items-center gap-3">
            <div
              className={`rounded-lg p-2 ${isBetter ? "bg-green-500/10" : "bg-red-400/10"}`}
            >
              {isBetter ? (
            ...
            <div>
              ...
              <p
                className={`text-sm font-bold ${isBetter ? "text-green-500" : "text-red-400"}`}
              >
          ...
          <div className="col-span-2">
            ...
            <p
              className={`text-sm font-bold ${(r.residualPct ?? 0) === 0 ? "text-green-500" : "text-yellow-500"}`}
            >
            ...
            <div className="col-span-2 flex items-center gap-2">
              <div className="flex-1">
                <p className="text-xs text-muted-foreground">
                  Portfolio Avg Slippage
                </p>
                <p
                  className={`text-sm font-bold ${multiDayResponse.avgSlippageBps < 0 ? "text-green-500" : "text-red-400"}`}
                >
                  {multiDayResponse.avgSlippageBps.toFixed(2)} bps
                </p>
              </div>
              <div
                className={`rounded-lg p-2 ${multiDayResponse.avgSlippageBps < 0 ? "bg-green-500/10" : "bg-red-400/10"}`}
              >
      ...
      <div className="grid grid-cols-12 gap-6 items-center">
        ...
        <div className="col-span-2">
          ...
          <p className="text-sm font-bold text-foreground">
            ...
            {(params.totalQuantity / 1000).toFixed(0)}K
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            {((results.executedQuantity / params.totalQuantity) * 100).toFixed(
              1,
            )}
            %
          </p>
        ...
        <div className="col-span-2">
          ...
          <p
            className={`text-sm font-bold ${results.executionVsVWAPPct < 0 ? "text-green-500" : "text-red-500"}`}
          >
