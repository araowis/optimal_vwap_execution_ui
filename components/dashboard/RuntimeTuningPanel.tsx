...
import {
  ChevronDown, ChevronUp, RefreshCw, RotateCcw, Send, Diff,
  Loader2, AlertTriangle, Wifi, WifiOff,
} from 'lucide-react';
import { RuntimeTuningParams, RuntimeTuningProfile } from '@/lib/vwap-server-types';
...
const PARAM_GROUPS: ParamGroup[] = [
  ...
  },
  {
    ...
    dot: 'bg-amber-500',
    fields: [
      'rawTrendWeightGap', 'rawTrendWeightConsistency', 'rawTrendWeightReturn',
      'upTrendScoreThreshold', 'upTrendConsecBars',
      'downTrendScoreThreshold', 'downTrendConsecBars',
      'maxTrendAccel', 'minTrendAccel', 'trendAccelScaleUp', 'trendAccelScaleDown',
    ],
  },
  {
    label: 'EWMA',
  ...
  },
  {
    ...
    dot: 'bg-emerald-500',
    fields: ['adjustmentClampLo', 'adjustmentClampHi', 'barStructAlpha', 'scaleFactorHi', 'scaleFactorLo'],
  },
  {
    label: 'Liquidity',
...
export default function RuntimeTuningPanel({
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const msg = e?.message?.includes('400') ? 'Validation error — check param constraints' : 'Failed to apply tuning';
      toast.error(msg);
    } finally {
      setApplying(false);
    }
      ...
      <div className="px-3 pt-3 pb-2 space-y-2">
        {noContext ? (
          <div className="flex items-center gap-2 px-3 py-2 bg-amber-500/8 border border-amber-500/20 rounded-md">
            ...
            <p className="text-[10px] text-amber-400 font-medium">Select a client and instrument to tune</p>
          </div>
        ) : (
          <div className="flex items-center justify-between px-2.5 py-1.5 bg-secondary/30 rounded-md border border-border/50">
            <div className="flex items-center gap-2 min-w-0">
              <div className={cn('w-1.5 h-1.5 rounded-full flex-shrink-0', loading ? 'bg-amber-400 animate-pulse' : 'bg-emerald-400')} />
              <p className="text-[10px] font-mono text-muted-foreground truncate" title={tuningKey!}>
                <span className="text-foreground/60">key:</span> {tuningKey}
              </p>
            </div>
            {lastSynced && (
              <p className="text-[9px] text-muted-foreground/50 flex-shrink-0 ml-2">
                {lastSynced.toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
              </p>
            )}
          </div>
        )}
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2">
        {loading && !noContext ? (
          <div className="flex flex-col items-center justify-center py-10 gap-3">
            ...
            <p className="text-[10px] text-muted-foreground/50 uppercase tracking-widest">Loading params…</p>
          </div>
        ) : (
          ...
          <div className="w-full bg-card border-t border-border shadow-xl animate-in slide-in-from-bottom-4 duration-200 max-h-[70%] flex flex-col rounded-t-xl">
            <div className="flex items-center justify-between px-4 py-3 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-foreground uppercase tracking-wider">Live Diff</span>
                <span className="text-[9px] font-mono text-muted-foreground">vs factory defaults</span>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-0.5">
              {Object.keys(diffData).length === 0 ? (
                <div className="flex flex-col items-center py-8 gap-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">All params at factory defaults</p>
                </div>
              ) : (
