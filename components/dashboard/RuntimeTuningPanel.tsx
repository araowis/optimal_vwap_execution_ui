...
> = {
  lambdaMultiplier: { desc: "Brute force method of scaling the risk parameter.", typical: "0.5 – 5.0", recommended: "1.0 – 2.0" },
  adjustmentClampLo: { desc: "Max fraction by which the engine can decelerate below the scheduled target. Safety floor.", typical: "-0.5 – 0.0", recommended: "-0.15" },
  adjustmentClampHi: { desc: "Max fraction by which the engine can accelerate above the scheduled target. Prevents over-execution.", typical: "0.0 – 0.5", recommended: "0.15" },
  rawTrendWeightGap: { desc: "The weight assigned to the price-VWAP gap in the final trend score.", typical: "0.1 – 0.5", recommended: "0.33" },
  rawTrendWeightConsistency: { desc: "The weight assigned to trend consistency, which measures how consistently prices have been moving in one direction over the recent lookback period.", typical: "0.1 – 0.5", recommended: "0.33" },
  rawTrendWeightReturn: { desc: "The weight assigned to the trend return, which is the difference of the session open price and the last recorded price.", typical: "0.1 – 0.5", recommended: "0.34" },
  ewmaFastOldWeight: { desc: "Fraction of the previous trend score retained while the model is still in its fast-adaption phase (beginning of the day).", typical: "0.1 – 0.3", recommended: "0.25" },
  ewmaSlowOldWeight: { desc: "fraction of the previous trend score retained after the model has accumulated sufficient history and transitions to steady-state smoothing (midway, when we have more data).", typical: "0.5 – 0.8", recommended: "0.6" },
  ...
  trendAccelScaleUp: { desc: "Multiplier applied when scaling up execution speed on positive trends. USE THIS WHEN IN UPTREND.", typical: "0.5 – 2.0", recommended: "1.0" },
  trendAccelScaleDown: { desc: "Multiplier applied when slowing execution speed on negative trends. USE THIS WHEN IN DOWNTREND.", typical: "0.5 – 2.0", recommended: "1.0" },
  ...
  barStructAlpha: { desc: "Assumed alpha from exploiting micro-structure within individual trading bars. Bar structure is the microstructure characteristics of an OHLCV bar, such as body size, range, close location within the range and wick/tail behaviour.", typical: "0.1 – 1.0", recommended: "0.5" },
  scaleFactorHi: { desc: "Aggressive pacing multiplier during high-confidence favorable conditions. USE THIS WHEN YOU HAVE HIGH CONFIDENCE IN THE TREND.", typical: "1.0 – 2.0", recommended: "1.2" },
  scaleFactorLo: { desc: "Passive pacing multiplier during unfavorable conditions or wide spreads. USE THIS WHEN YOU ARE IN A DOWNTREND.", typical: "0.1 – 1.0", recommended: "0.8" },
  executionScoreHistWeight: { desc: "The weight assigned to historically predicted volume from the pretrade engine. What weight remains is assigned to the realtime adaptive volume based on incoming data.", typical: "0.1 – 0.9", recommended: "0.5" },
  ...
  liquidityFactorBase: { desc: "Baseline participation multiplier applied even when liquidity is poor.", typical: "0.5 – 2.0", recommended: "1.0" },
  liquidityFactorScale: { desc: "Incremental multiplier added as liquidity improves.", typical: "0.5 – 2.0", recommended: "1.0" },
