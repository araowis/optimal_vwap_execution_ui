...
async function startL2Stream() {
    try {
        ...
        const streamer = new UpstoxClient.MarketDataStreamerV3([INSTRUMENT_KEY], 'full');
        ...
        streamer.on('message', (data) => {
            ...
            if (feed && feed.ff && feed.ff.marketFF) {
                ...
                depth.buy.forEach(bid => {
                    console.log(`  Price: ${bid.price} | Qty: ${bid.quantity} | Orders: ${bid.orders}`);
                });

                console.log("\nSELL ORDERS (Asks):");
                depth.sell.forEach(ask => {
                    console.log(`  Price: ${ask.price} | Qty: ${ask.quantity} | Orders: ${ask.orders}`);
                });
            } else {
